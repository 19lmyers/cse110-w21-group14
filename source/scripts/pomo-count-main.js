...
function show() {
  ...
  if(taskList.style.visibility == 'visible') {
...
class Task {
  constructor (name, count) {
    this.createTask(name, count);
  }
  createTask(name, count) {
    ...
    let estimated = document.createElement('input');
    ...
    myList.appendChild(estimated);
    myList.appendChild(editButton);
    myList.appendChild(deleteButton);

    check.addEventListener('click', () => {
      if(check.checked == true) {
        //NEED TO MKAE IT STRIKE THROUGH THIS ELEMENT IN THE LIST
...
function insert() {
  ...
  let count = document.getElementById('estimated-count');
  if(insert.value != "") {
    new Task(insert.value, count.value);
    let TaskList = {
      name: insert.value,
      estimated: count.value,
      actual: 0,
      done: false,
    }
...
taskList.forEach(task => {
  new Task(task.name, task.estimated);
});
